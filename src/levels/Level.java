...
public class Level {
    ...
    private void initializeWavesFromStorage() {
        ...
        if (levelInfo == null) {
            System.err.println("CRITICAL: No level data found in storage for level " + levelNumber);
            levelComplete = true;
            return;
        }
    ...
    private void handleCollisions() {
        ...
        for (Entity entity : currentEntities) {
            if (entity instanceof Titan && entity.isActive()) {
                ...
                for (Entity target : currentEntities) {
                    if (target instanceof SpikeTrap && target.isActive()) {
    ...
    public void startNextWave() {
        ...
        if (currentWaveIndex < waves.size()) {
            ...
            System.out.println("All waves for level " + levelNumber + " initiated.");
        }
    }
    ...
    public void draw(Graphics g) {
        ...
        for (Entity entity : entitiesToDraw) {
            if (entity instanceof BlockerWall || entity instanceof SpikeTrap) {
                entity.draw(g);
            }
        }
        for (Entity entity : entitiesToDraw) {
    ...
    public int getCurrentWaveNumber() { return (currentWave != null) ? currentWave.getWaveNumber() : 0; }
    ...
    public String getCurrentWaveStatus() {
        ...
        if (currentWave.isSpawningComplete()) {
            if (titansExist) {
                return "Clear remaining titans!";
            } else {
                if (currentWaveIndex >= waves.size() - 1) {
                    return "All waves cleared!";
                } else {
                    return "Wave Cleared! Next wave soon...";
                }
            }
